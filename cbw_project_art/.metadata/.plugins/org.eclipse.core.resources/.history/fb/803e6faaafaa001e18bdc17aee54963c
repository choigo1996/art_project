...
public class PurchaseController {
	...
	public ResponseEntity<Purchase> savePurchase(@RequestBody Purchase purchase) {
		return new ResponseEntity<Purchase>(
				purchaseService.savePurchase(purchase),HttpStatus.OK);
	}
	//모든 회원이 구매한 목록을 저장
	@PostMapping("list")
	public ResponseEntity<List<Purchase>> savePurchaseList(
		...
		for (Purchase purchase : purchaseList) {
			savedPurchaseList.add(purchaseService.savePurchase(purchase));
		}
	...
	public ResponseEntity<List<Purchase>> getAllPurchase() {
		return new ResponseEntity<List<Purchase>>(purchaseService.getAllPurchase(), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Purchase>> getPurchaseById(@PathVariable String loginId) {
		return new ResponseEntity<List<Purchase>>(
				purchaseService.getPurchaseById(loginId),HttpStatus.OK);
	}
}
